...
std::tuple<BPTree<ID>, BPTree<Name>, BPTree<Dob>, BPTree<Gender>, BPTree<Doj>, BPTree<Address>, BPTree<Qualification>> populate_from_file(string filename)
{
    BPTree<a> tree1;
    BPTree<b> tree2;
    BPTree<c> tree3;
    ...
    string typec = typeid(c).name();
    string typed = typeid(d).name();
    string typee = typeid(e).name();
    string typef = typeid(f).name();
    string typeg = typeid(g).name();

    // regex for type
    smatch stype;
    regex type_regex("([a-zA-Z]+)");
    regex_search(typea, stype, type_regex);
    typea = stype.str(0);

    regex_search(typeb, stype, type_regex);
    typeb = stype.str(0);

    regex_search(typec, stype, type_regex);
    typec = stype.str(0);

    regex_search(typed, stype, type_regex);
    typed = stype.str(0);

    regex_search(typee, stype, type_regex);
    typee = stype.str(0);

    regex_search(typef, stype, type_regex);
    typef = stype.str(0);

    regex_search(typeg, stype, type_regex);
    typeg = stype.str(0);
    ...
    while (regex_search(str, match, rid))
    {
        ...
        if (typef == "Address")
        {
            ...
            tree6.insert(Address, reference);
        }
        // cout << "Address: " << Address << endl;

        if (typeg == "Qualification")
        {
            ...
            tree7.insert(Qualification, reference);
        }
    ...
    file.close();
    return make_tuple(tree1, tree2, tree3, tree4, tree5, tree6, tree7);
}
...
int main()
{
    ...
    BPTree<Gender> tree4;
    BPTree<Doj> tree5;
    BPTree<Address> tree6;
    BPTree<Qualification> tree7;

    tie(tree1, tree2, tree3, tree4, tree5, tree6, tree7) = populate_from_file("Fall2022DSDataFile001.txt");
